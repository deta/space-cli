...
var (
	...
	logsProjectDir string
	logsMicro      string
	logsCmd        = &cobra.Command{
		Use:   "logs [flags]",
		Short: "view runtime logs of Builder instance",
		RunE:  logs,
	}
)

func init() {
	logsCmd.Flags().StringVarP(&logsProjectID, "id", "i", "", "project id of project to push")
	logsCmd.Flags().StringVarP(&logsProjectDir, "dir", "d", "./", "src of project to push")
	logsCmd.Flags().StringVarP(&logsMicro, "micro", "m", "", "micro to get logs for")
	rootCmd.AddCommand(logsCmd)
}

func selectLogsProjectID() (string, error) {
...
func logs(cmd *cobra.Command, args []string) error {
	...
	logsProjectDir = filepath.Clean(logsProjectDir)

	runtimeManager, err := runtime.NewManager(&logsProjectDir, false)
	if err != nil {
		return err
	}
	...
	if isProjectInitialized {
		...
		logsProjectID = projectMeta.ID
	} else if isFlagEmpty(logsProjectID) {
		logger.Printf("No project was found in the current directory.\n\n")
		logger.Printf("You can still get logs by providing a valid Project ID.\n\n")

		logsProjectID, err = selectLogsProjectID()
		if err != nil {
			return fmt.Errorf("problem while trying to get project id from prompt, %v", err)
		}
	}

	logger.Printf("Getting logs for micro %s", logsMicro)

	i, err := client.GetBuilderInstance(&api.GetBuilderInstanceRequest{AppID: logsProjectID})
	if err != nil {
		logger.Printf("%s Error: %v\n", emoji.ErrorExclamation, err)
		logger.Printf("Create or update your Builder instance with %s\n", styles.Code("space release"))
		return nil
	}

	logs, err := client.GetInstanceLogs(&api.GetInstanceLogsRequest{InstanceID: i.ID, Micro: logsMicro})
	if err != nil {
		logger.Printf("%s Error: %v\n", emoji.ErrorExclamation, err)
		return nil
	}

	logger.Printf("Logs for micro %s", logsMicro)
	...
	isSpacefilePrsent, err := spacefile.IsSpacefilePresent(logsProjectDir)
	if err != nil {
	...
	if len(spacefileErrors) > 0 {
		...
		logger.Println(styles.Error("\nPlease try to fix the issues with your Spacefile before getting logs."))
		return nil
	} else {
		logValidationErrors(s, spacefileErrors)
		logger.Printf(styles.Green("\nYour Spacefile looks good, proceeding with getting your logs!!\n"))
	}
	...
	buildSpinnerInput := spinner.Input{
		...
		Request: func() tea.Msg {
			r, err := client.CreateBuild(&api.CreateBuildRequest{AppID: logsProjectID})
	...
	// push spacefile
	raw, err := spacefile.OpenRaw(logsProjectDir)
	if err != nil {
		return err
	}
	...
	// push discovery file
	df, err := discovery.Open(logsProjectDir)
	if err != nil {
	...
	zippedCode, err := runtimeManager.ZipDir(logsProjectDir)
	if err != nil {
		return err
	}
	...
	if err != nil {
		logger.Printf(styles.Errorf("\n%s Failed to check if push succeded. Please check %s if a new revision was created successfully.", emoji.ErrorExclamation, styles.Codef("%s/%s/develop", builderUrl, logsProjectID)))
		return nil
	}
